...
def call_stream(
    ...
    """call() 的 streaming 版本（OpenRouter SSE，OpenAI 相容格式）。

    是 generator，依序 yield tuple：
      ("delta", text_piece)  逐塊吐出的答案文字
      ("usage", usage_dict)  串流結束時的用量（格式同 call() 的 usage）

    usage 來自 OpenRouter 最後一個帶 `usage` 的 chunk（需 stream + usage.include）。
    """
    payload = {
        "model": model,
        "max_tokens": max_tokens,
    ...
    with client.stream("POST", OPENROUTER_URL, json=payload) as resp:
        resp.raise_for_status()
        for line in resp.iter_lines():
            if not line or not line.startswith("data:"):
                continue  # 跳過 SSE 註解行（: OPENROUTER PROCESSING）與空行
            ...
            if u:
                ...
                usage = {
                    "input": max(0, prompt_tokens - cached),
                    "cached_input": cached,
                    "output": completion_tokens,
                    "total": prompt_tokens + completion_tokens,
                }
